...
def load_sweep_data():
    ...
    with open('simulator/results/sim_sweep_zipf/data/sweep_results.json', 'r') as f:
        return json.load(f)
...
def plot_pending_transactions_overlay():
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig('simulator/results/sim_sweep_zipf/figs/pending_transactions_overlay.png', 
...
def plot_success_transactions_overlay():
    ...
    try:
        ...
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
        plt.savefig('simulator/results/sim_sweep_zipf/figs/success_transactions_overlay.png', 
                   dpi=300, bbox_inches='tight')
        plt.close()
...
def plot_failure_transactions_overlay():
    ...
    try:
        ...
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
        plt.savefig('simulator/results/sim_sweep_zipf/figs/failure_transactions_overlay.png', 
                   dpi=300, bbox_inches='tight')
        plt.close()
...
def plot_transaction_status_chart(ax, data):
    """Create a line chart showing failed/success/pending data vs Zipf parameter"""
    try:
        individual_results = data['individual_results']
        ...
        for result in individual_results:
            zipf_parameters.append(result['zipf_parameter'])
        ...
        ax.plot(zipf_parameters, success_counts, 'go-', linewidth=2, markersize=6, label='Success')
        ax.plot(zipf_parameters, failure_counts, 'ro-', linewidth=2, markersize=6, label='Failed')
        ax.plot(zipf_parameters, pending_counts, 'yo-', linewidth=2, markersize=6, label='Pending')
        
        ax.set_title('Transaction Status vs Zipf Parameter')
...
def plot_sweep_summary():
    """Plot summary statistics across all Zipf parameters"""
    try:
        data = load_sweep_data()
        sweep_summary = data['sweep_summary']
        ...
        zipf_parameters = sweep_summary['zipf_parameters']
        ...
        ax1.plot(zipf_parameters, total_transactions, 'bo-', linewidth=2, markersize=6)
        ax1.set_title('Total Transactions vs Zipf Parameter')
        ax1.set_xlabel('Zipf Parameter')
        ax1.set_ylabel('Total Transactions')
        ax1.grid(True, alpha=0.3)
        
        # Plot 2: CAT transactions
        ax2.plot(zipf_parameters, cat_transactions, 'ro-', linewidth=2, markersize=6)
        ax2.set_title('CAT Transactions vs Zipf Parameter')
        ax2.set_xlabel('Zipf Parameter')
        ...
        ax4.plot(zipf_parameters, cat_transactions, 'ro-', linewidth=2, markersize=6, label='CAT Transactions')
        ax4.plot(zipf_parameters, regular_transactions, 'go-', linewidth=2, markersize=6, label='Regular Transactions')
        ax4.set_title('Transaction Distribution by Zipf Parameter')
        ax4.set_xlabel('Zipf Parameter')
        ...
        plt.tight_layout()
        plt.savefig('simulator/results/sim_sweep_zipf/figs/sweep_summary.png', 
                   dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    os.makedirs('simulator/results/sim_sweep_zipf/figs', exist_ok=True)
    
    print("Generating sweep Zipf distribution simulation plots...")
    ...
    print("Sweep Zipf distribution simulation plots generated successfully!")
