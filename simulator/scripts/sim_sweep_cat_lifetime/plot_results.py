...
def load_sweep_data():
    ...
    with open('simulator/results/sim_sweep_cat_lifetime/data/sweep_results.json', 'r') as f:
        return json.load(f)
...
def plot_pending_transactions_overlay():
    ...
    try:
        ...
        for i, result in enumerate(individual_results):
            cat_lifetime = result['cat_lifetime']
            ...
            # Plot with color based on CAT lifetime
            plt.plot(heights, counts, color=colors[i], alpha=0.7, 
                    label=f'CAT lifetime: {cat_lifetime} blocks', linewidth=1.5)
        
        plt.title('Pending Transactions by Height (Chain 1) - CAT Lifetime Sweep')
        ...
        plt.tight_layout()
        plt.savefig('simulator/results/sim_sweep_cat_lifetime/figs/pending_transactions_overlay.png', 
...
def plot_success_transactions_overlay():
    ...
    try:
        ...
        plt.title('Success Transactions by Height (Chain 1) - CAT Lifetime Sweep')
        plt.xlabel('Block Height')
        ...
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
        plt.savefig('simulator/results/sim_sweep_cat_lifetime/figs/success_transactions_overlay.png', 
                   dpi=300, bbox_inches='tight')
        plt.close()
...
def plot_failure_transactions_overlay():
    ...
    try:
        ...
        plt.title('Failure Transactions by Height (Chain 1) - CAT Lifetime Sweep')
        plt.xlabel('Block Height')
        ...
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
        plt.savefig('simulator/results/sim_sweep_cat_lifetime/figs/failure_transactions_overlay.png', 
                   dpi=300, bbox_inches='tight')
        plt.close()
...
def plot_transaction_status_chart(ax, data):
    """Create a line chart showing failed/success/pending data vs CAT lifetime"""
    try:
        individual_results = data['individual_results']
        ...
        for result in individual_results:
            cat_lifetimes.append(result['cat_lifetime'])
        ...
        ax.plot(cat_lifetimes, success_counts, 'go-', linewidth=2, markersize=6, label='Success')
        ax.plot(cat_lifetimes, failure_counts, 'ro-', linewidth=2, markersize=6, label='Failed')
        ax.plot(cat_lifetimes, pending_counts, 'yo-', linewidth=2, markersize=6, label='Pending')
        
        ax.set_title('Transaction Status vs CAT Lifetime')
        ax.set_xlabel('CAT Lifetime (blocks)')
...
def plot_sweep_summary():
    """Plot summary statistics across all CAT lifetimes"""
    try:
        data = load_sweep_data()
        sweep_summary = data['sweep_summary']
        ...
        cat_lifetimes = sweep_summary['cat_lifetimes']
        ...
        ax1.plot(cat_lifetimes, total_transactions, 'bo-', linewidth=2, markersize=6)
        ax1.set_title('Total Transactions vs CAT Lifetime')
        ax1.set_xlabel('CAT Lifetime (blocks)')
        ax1.set_ylabel('Total Transactions')
        ax1.grid(True, alpha=0.3)
        
        # Plot 2: CAT transactions
        ax2.plot(cat_lifetimes, cat_transactions, 'ro-', linewidth=2, markersize=6)
        ax2.set_title('CAT Transactions vs CAT Lifetime')
        ax2.set_xlabel('CAT Lifetime (blocks)')
        ...
        ax4.plot(cat_lifetimes, cat_transactions, 'ro-', linewidth=2, markersize=6, label='CAT Transactions')
        ax4.plot(cat_lifetimes, regular_transactions, 'go-', linewidth=2, markersize=6, label='Regular Transactions')
        ax4.set_title('Transaction Distribution by CAT Lifetime')
        ax4.set_xlabel('CAT Lifetime (blocks)')
        ...
        plt.tight_layout()
        plt.savefig('simulator/results/sim_sweep_cat_lifetime/figs/sweep_summary.png', 
                   dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    os.makedirs('simulator/results/sim_sweep_cat_lifetime/figs', exist_ok=True)
    
    print("Generating sweep CAT lifetime simulation plots...")
    ...
    print("Sweep CAT lifetime simulation plots generated successfully!")
