...
delay remains constant at 0.5 seconds while the block interval varies.
"""
...
def load_sweep_data():
    ...
    with open('simulator/results/sim_sweep_block_interval_constant_delay/data/sweep_results.json', 'r') as f:
        return json.load(f)
...
def plot_pending_transactions_overlay():
    ...
    try:
        ...
        for i, result in enumerate(individual_results):
            ...
            # Plot with color based on block interval
            plt.plot(heights, counts, color=colors[i], alpha=0.7, 
                    label=f'Block Interval: {block_interval:.3f}s', linewidth=1.5)
        
        plt.title('Pending Transactions by Height (Chain 1) - Block Interval Sweep')
        ...
        plt.tight_layout()
        plt.savefig('simulator/results/sim_sweep_block_interval_constant_delay/figs/pending_transactions_overlay.png', 
...
def plot_success_transactions_overlay():
    ...
    try:
        ...
        plt.title('Success Transactions by Height (Chain 1) - Block Interval Sweep')
        plt.xlabel('Block Height')
        ...
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
        plt.savefig('simulator/results/sim_sweep_block_interval_constant_delay/figs/success_transactions_overlay.png', 
                   dpi=300, bbox_inches='tight')
        plt.close()
...
def plot_failure_transactions_overlay():
    ...
    try:
        ...
        plt.title('Failure Transactions by Height (Chain 1) - Block Interval Sweep')
        plt.xlabel('Block Height')
        ...
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
        plt.savefig('simulator/results/sim_sweep_block_interval_constant_delay/figs/failure_transactions_overlay.png', 
                   dpi=300, bbox_inches='tight')
        plt.close()
...
def plot_transaction_status_chart(ax, data):
    """Create a line chart showing failed/success/pending data vs block interval"""
    try:
        individual_results = data['individual_results']
        ...
        for result in individual_results:
            block_intervals.append(result['block_interval'])
        ...
        ax.plot(block_intervals, success_counts, 'go-', linewidth=2, markersize=6, label='Success')
        ax.plot(block_intervals, failure_counts, 'ro-', linewidth=2, markersize=6, label='Failed')
        ax.plot(block_intervals, pending_counts, 'yo-', linewidth=2, markersize=6, label='Pending')
        
        ax.set_title('Transaction Status vs Block Interval')
        ax.set_xlabel('Block Interval (seconds)')
...
def plot_sweep_summary():
    """Plot summary statistics across all block intervals"""
    try:
        data = load_sweep_data()
        sweep_summary = data['sweep_summary']
        ...
        block_intervals = sweep_summary['block_interval']
        ...
        ax1.plot(block_intervals, total_transactions, 'bo-', linewidth=2, markersize=6)
        ax1.set_title('Total Transactions vs Block Interval')
        ax1.set_xlabel('Block Interval (seconds)')
        ax1.set_ylabel('Total Transactions')
        ax1.grid(True, alpha=0.3)
        
        # Plot 2: CAT transactions
        ax2.plot(block_intervals, cat_transactions, 'ro-', linewidth=2, markersize=6)
        ax2.set_title('CAT Transactions vs Block Interval')
        ax2.set_xlabel('Block Interval (seconds)')
        ...
        ax4.plot(block_intervals, cat_transactions, 'ro-', linewidth=2, markersize=6, label='CAT Transactions')
        ax4.plot(block_intervals, regular_transactions, 'go-', linewidth=2, markersize=6, label='Regular Transactions')
        ax4.set_title('Transaction Distribution by Block Interval')
        ax4.set_xlabel('Block Interval (seconds)')
        ...
        plt.tight_layout()
        plt.savefig('simulator/results/sim_sweep_block_interval_constant_delay/figs/sweep_summary.png', 
                   dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    os.makedirs('simulator/results/sim_sweep_block_interval_constant_delay/figs', exist_ok=True)
    
    print("Generating sweep block interval constant delay simulation plots...")
    ...
    print("Sweep block interval constant delay simulation plots generated successfully!")
