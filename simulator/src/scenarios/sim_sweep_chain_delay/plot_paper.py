...
def plot_cat_success_percentage_violin_by_simulation(data: Dict[str, Any], param_name: str, results_dir: str, sweep_type: str, plot_config: Dict[str, Any]) -> None:
    ...
    for each chain delay, using the final values from each run.
    """
    try:
        individual_results = data['individual_results']
        ...
        for sim_index in range(num_simulations):
            ...
            if final_percentages:
                ...
                labels.append(f'{param_value:.1f}')
        ...
        # Customize plot
        plt.xlabel('Chain Delay (blocks)')
        plt.ylabel('CAT Success Percentage (%)')
        plt.title(f'CAT Success Percentage Distribution by Chain Delay - {create_sweep_title(param_name, sweep_type)}')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
