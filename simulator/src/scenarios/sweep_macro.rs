...
/// * `sweep_parameters` - (Optional) The type of sweep parameters to use (e.g., CatRateSweepParameters)
...
macro_rules! define_sweep_config {
    ...
    (
        $dir_name:expr,
        $config_name:ident,
        sweep_parameters = $sweep_params_type:ty,
        ...
        impl crate::config::ValidateConfig for $config_name {
            fn validate_common(&self) -> Result<(), crate::config::ConfigError> {
                crate::config::validate_common_fields(&self.account_config, &self.transaction_config, &self.network_config, &self.repeat_config)?;
        ...
        impl crate::scenarios::sweep_runner::SweepConfigTrait for $config_name {
            ...
            fn get_repeat_config(&self) -> Option<&crate::config::RepeatConfig> { Some(&self.repeat_config) }
        }
    ...
    // Old format with generic sweep parameters (for backward compatibility)
    (
        $dir_name:expr,
        $config_name:ident,
        validate_sweep_specific = $validate_block:expr
