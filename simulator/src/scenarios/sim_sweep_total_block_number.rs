...
pub struct SweepTotalBlockNumberConfig {
...
impl ValidateConfig for SweepTotalBlockNumberConfig {
...
impl SweepConfigTrait for SweepTotalBlockNumberConfig {
...
fn load_config() -> Result<SweepTotalBlockNumberConfig, crate::config::ConfigError> {
    let config_str = fs::read_to_string("simulator/src/scenarios/config_sweep_total_block_number.toml")?;
    let config: SweepTotalBlockNumberConfig = toml::from_str(&config_str)?;
    config.validate()?;
    Ok(config)
}
