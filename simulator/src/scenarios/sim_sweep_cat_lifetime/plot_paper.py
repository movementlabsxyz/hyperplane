#!/usr/bin/env python3
"""
Paper-specific plotting script for CAT Lifetime Sweep Simulation
...
def plot_cat_success_percentage_violin(data: Dict[str, Any], param_name: str, results_dir: str, sweep_type: str, plot_config: Dict[str, Any]) -> None:
    ...
    try:
        ...
        plt.title(f'CAT Success Percentage Distribution by CAT Lifetime - {create_sweep_title(param_name, sweep_type)}')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
...
def plot_tx_pending_cat_postponed_violin(data: Dict[str, Any], param_name: str, results_dir: str, sweep_type: str, plot_config: Dict[str, Any]) -> None:
    ...
    try:
        ...
        for sim_index, result in enumerate(individual_results):
            ...
            if DEBUG_MODE:
                print(f"DEBUG: Simulation {sim_index} (CAT Lifetime {param_value:.3f}):")
        ...
        if DEBUG_MODE:
            ...
            for i, (values, label) in enumerate(zip(violin_data, labels)):
                print(f"Simulation {i} (CAT Lifetime {label}): {len(values)} values, min={min(values)}, max={max(values)}, mean={np.mean(values):.2f}")
                if len(values) > 10:
        ...
        for i, (values, label) in enumerate(zip(violin_data, labels)):
            violin_plot_data['data'].append({
                'simulation_index': i,
                'parameter_value': float(label),
                'final_values': [int(v) for v in values],  # Convert to native Python int
                ...
                'min_value': int(np.min(values)),  # Convert to native Python int
                'max_value': int(np.max(values))  # Convert to native Python int
            })
        ...
        plt.title(f'CAT Pending Postponed Transactions Distribution by CAT Lifetime - {create_sweep_title(param_name, sweep_type)}')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
...
def plot_tx_pending_cat_resolving_violin(data: Dict[str, Any], param_name: str, results_dir: str, sweep_type: str, plot_config: Dict[str, Any]) -> None:
    ...
    try:
        ...
        plt.title(f'CAT Pending Resolving Transactions Distribution by CAT Lifetime - {create_sweep_title(param_name, sweep_type)}')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
...
def plot_tx_pending_regular_violin(data: Dict[str, Any], param_name: str, results_dir: str, sweep_type: str, plot_config: Dict[str, Any]) -> None:
    ...
    try:
        ...
        plt.title(f'Regular Pending Transactions Distribution by CAT Lifetime - {create_sweep_title(param_name, sweep_type)}')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
...
def main():
    """Main function to generate paper-specific plots for CAT lifetime sweep simulation."""
    # Configuration for this specific sweep
    param_name = 'cat_lifetime'
    results_dir = '../../../results/sim_sweep_cat_lifetime'
    ...
    try:
        ...
        results_dir_name = results_dir.split('/')[-1]  # Extract 'sim_sweep_cat_lifetime'
