...
struct TestNodeStateV11 {
    ...
    subblock_sender: mpsc::Sender<SubBlockMessage>,
...
async fn run_processor_v11(state: Arc<Mutex<TestNodeStateV11>>) {
    ...
    loop {
        ...
        for chain_id in &state.registered_chains {
            ...
            if let Err(e) = state.subblock_sender.send(SubBlockMessage { subblock }).await {
...
/// - Still keeps the simple mutex pattern for state
#[tokio::test]
async fn test_mutex_concurrent_access_v12() {
    println!("\n=== Starting test_mutex_concurrent_access_v12 ===");
    
    // TODO: Implement test with proper node setup
    println!("=== Test completed successfully ===\n");
}