...
async fn test_hs_to_cl_status_update() {
    ...
    // Wait for the confirmation layer to process the messages
    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;

    // Verify that subblocks for both chains contain the status update message
    let subblock1 = cl.get_subblock(chain1.clone(), BlockId("0".to_string())).await.expect("Failed to get subblock for chain1");
    let subblock2 = cl.get_subblock(chain2.clone(), BlockId("0".to_string())).await.expect("Failed to get subblock for chain2");
