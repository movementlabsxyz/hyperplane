use hyperplane::{
    types::{Transaction, TransactionId, TransactionStatus, CATStatusUpdate, ChainId, BlockId, CLTransaction, CATId},
    hyper_ig::{HyperIG, HyperIGNode},
    hyper_scheduler::{HyperSchedulerNode, HyperScheduler},
...
async fn test_cat_complete_flow() {
    ...
    let mut cl = ConfirmationNode::new();
    ...
    cl.register_chain(chain_id.clone()).await.expect("Failed to register chain");

    // Connect components
    println!("[test.Setup] Connecting components...");
    hig.set_hyper_scheduler(Box::new(hs.clone()));
    hs.set_confirmation_layer(Box::new(cl));

    // Register chain in HS
    println!("[test.Setup] Registering chain in HS...");
    hs.set_chain_id(chain_id.clone());
    ...
    hs.confirmation_layer_mut().unwrap().submit_transaction(CLTransaction {
        id: cat_tx.id.clone(),
    ...
    let current_block = hs.confirmation_layer_mut().unwrap().get_current_block().await.expect("Failed to get current block");
    ...
    for block_num in 0..current_block_num {
        ...
        let subblock = hs.confirmation_layer_mut().unwrap().get_subblock(chain_id.clone(), block_id.clone())
            .await
    ...
    let status = hig.get_transaction_status(cat_tx.id.clone())
        .await
        .expect("Failed to get transaction status");
    assert!(matches!(status, TransactionStatus::Pending));

    let proposed_status = hig.get_proposed_status(cat_tx.id.clone())
        .await
    ...
    hig.send_cat_status_proposal(CATId(cat_tx.id.0.clone()), CATStatusUpdate::Success)
        .await
    ...
    let hs_status = hs_ref.get_cat_status(CATId(cat_tx.id.0.clone())).await.unwrap();
    assert_eq!(hs_status, CATStatusUpdate::Success);

    // HS sends the status update to CL
    println!("[test.HS] Sending status update to CL...");
    hs.send_cat_status_update(CATId(cat_tx.id.0.clone()), CATStatusUpdate::Success)
        .await
