...
#[async_trait]
pub trait ConfirmationLayer {
    /// Register a new chain to receive subBlocks
    /// Returns the block ID at which registration will take effect
    ...
    /// Get the subBlock for a specific chain at a specific block
    ...
    /// Set the block interval (time between blocks)
